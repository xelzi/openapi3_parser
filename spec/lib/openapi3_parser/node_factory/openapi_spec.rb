...
RSpec.describe Openapi3Parser::NodeFactory::Openapi do
  ...
  it_behaves_like "node object factory", Openapi3Parser::Node::Openapi do
    ...
    let(:node_context) do
      node_factory_context_to_node_context(input, node_factory_context)
    end
  end

  context "when input is nil" do
    subject(:factory) { described_class.new(input, node_factory_context) }
    let(:input) { nil }
    let(:node_factory_context) { create_node_factory_context }
    let(:node_context) do
      node_factory_context_to_node_context(input, node_factory_context)
  ...
  describe "servers" do
    subject(:node) do
      ...
      node_context = node_factory_context_to_node_context(input,
                                                          node_factory_context)
      described_class.new(input, node_factory_context)
                     .node(node_context)
    end
